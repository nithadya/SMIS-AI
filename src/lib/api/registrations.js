...
// Create student record from completed registration
export const createStudentFromRegistration = async (enrollmentId) => {
  try {
    const user = getCurrentUser();
    if (!user) throw new Error('Not authenticated');
    ...
    const regData = enrollment.registration_data;
    if (!regData) {
      throw new Error('Registration data not found');
    }

    // Get batch details for student ID generation
    const { data: batch, error: batchError } = await supabase
      .from('batches')
      .select('id, batch_code, program_id')
    ...
    const studentData = {
      ...
      program_id: regData.selected_program_id,
      ...
      created_by: user.email,
      updated_by: user.email
      // Note: student_id will be auto-generated by the trigger
    };
    ...
    console.error('Error in createStudentFromRegistration:', error);
    return { data: null, error: error.message };
  }
}; 